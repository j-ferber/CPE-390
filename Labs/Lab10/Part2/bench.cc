...
template <typename Func>
void benchmark_array2(const char msg[], Func f, uint64_t a[], uint64_t n,
                      uint64_t k) {
  cout << msg << ":\t";
  for (int i = 0; i < tries; i++) {
    clock_t t0 = clock();
...
template <typename Func>
void benchmark_array3(const char msg[], Func f, uint64_t crc, uint64_t a[],
                      uint64_t n) {
  cout << msg << ":\t";
  for (int i = 0; i < tries; i++) {
    clock_t t0 = clock();
...
int main() {
  ...
  xor_crypt(msg1, msg1len,
            11);  // xor with the character whose code is 11 (control-k)
  cout << msg1 << '\n';
